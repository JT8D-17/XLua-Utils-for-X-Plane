...
function Persistence_Menu_Callbacks(itemref)
    for i=2,#Persistence_Menu_Items do
        if itemref == Persistence_Menu_Items[i] then
            ...
            if i == 13 then
                if UtilPersist_HasDrefFile == 0 then
                    Persistence_DrefFile_Write(XLuaUtils_Path.."datarefs.cfg")
                    Persistence_DrefFile_Read(XLuaUtils_Path.."datarefs.cfg")
                    Persistence_Menu_Watchdog(Persistence_Menu_Items,12)
                end
                if UtilPersist_HasDrefFile == 1 then
                    Persistence_Reload()
                end
            end
...
function Persistence_Menu_Watchdog(intable,index)
    ...
    if index == 13 then
        if UtilPersist_HasDrefFile == 0 then Menu_ChangeItemPrefix(Persistence_Menu_ID,index,"Generate Dataref File Template",intable)
        elseif UtilPersist_HasDrefFile == 1 then Menu_ChangeItemPrefix(Persistence_Menu_ID,index,"Reload Config & Dataref File (Drefs: "..(#Persistence_Datarefs-1)..")",intable) end
    end
end
...
function Persistence_FirstRun()
    Preferences_Write(Persistence_Config_Vars,XLuaUtils_PrefsFile)
    if FileExists(XLuaUtils_Path.."datarefs.cfg") then
