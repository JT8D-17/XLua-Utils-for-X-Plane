...
function Dataref_InitContainer(inputdref,outputtable)
    ...
    if dataref == nil then -- Check if dataref exists
        LogOutput("Dataref "..inputdref.." could not be found and is discarded.")
    else
        if XPLM.XPLMCanWriteDataRef(dataref) == 0 then -- Check if dataref is writable
            LogOutput("Dataref "..inputdref.." is not writable and is discarded.")
        else
            ...
            --PrintToConsole("Found "..outputtable[#outputtable][1].." (Type: "..outputtable[#outputtable][2].."; Values: "..table.concat(outputtable[#outputtable][3],",").."; Handle "..tostring(outputtable[#outputtable][5])..")")
        end
    end
end
--[[ Access a dataref ]]
local function Dataref_Access(intable,index,subtable,mode)
    ...
    if mode == "read" then
        ...
        if XPLM.XPLMGetDataRefTypes(dataref) == 32 then 
            ...
            local value = ffi.new("char["..size.."]") -- Define character array
            XPLM.XPLMGetDatab(dataref,ffi.cast("void *",value),0,size) -- Get byte array values from dataref
            Persistence_Datarefs[#Persistence_Datarefs][subtable][1] = ffi.string(value) -- Write dataref value to value subtable for dataref                       
        end        
        --PrintToConsole("Reading "..intable[index][1].." (Type: "..intable[index][2].."; Values: "..table.concat(intable[index][subtable],",")..")")
    end
    if mode == "write" then
        ...
        --PrintToConsole("Writing "..intable[index][1].." (Type: "..intable[index][2].."; Values: "..table.concat(intable[index][subtable],",")..")")
    end
end
...
function Dataref_Read(intable,subtable,filter)
    for j=2,#intable do
        if filter == "All" then -- Loop through all datatrefs
            Dataref_Access(intable,j,subtable,"read")
        end 
...
function Dataref_Write(intable,subtable,filter)
    for j=2,#intable do
        if filter == "All" then -- Loop through all datatrefs
            Dataref_Access(intable,j,subtable,"write")
        end 
        if filter == intable[j][1] then -- Update a single dataref
            Dataref_Access(intable,j,subtable,"write")
        end    
