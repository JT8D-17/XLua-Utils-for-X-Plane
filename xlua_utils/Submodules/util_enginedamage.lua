...
{"MainSwitch",0},              -- Main engine damage module switch
...
"All Off & Repair",         -- Item index: 11
}
--[[ Menu variables for FFI ]]
local EngineDamage_Menu_ID = nil
local EngineDamage_Menu_Pointer = ffi.new("const char")
--[[ Other variables ]]
local DispUnits = {CHT="",EGT="",ITT="",N1="%",N2="%",TRQ="Nm"} -- Display units for various properties
...
function EngineDamage_DebugWindow_Update()
    ...
    for i=1,Table_ValGet(EngineDamage_Drefs_Once,"Eng_Num",4,1) do
        ...
        for j=1,#EngineData[i] do
            if Table_ValGet(EngineDamage_Config_Vars,"DMG_"..EngineData[i][j][1],nil,2) == 1 then Debug_Window_ReplaceLine("ED_E"..i..EngineData[i][j][1],"  "..EngineData[i][j][1].." Damage: "..Table_ValGet(EngineData[i],EngineData[i][j][1],nil,6).."/"..Table_ValGet(EngineData[i],EngineData[i][j][1],nil,5)) end
        end
    end
end
--[[
...
function EngineDamage_ProfileAircraft()
    -- Determine units for some properties
    if Table_ValGet(EngineDamage_Drefs_Once,"Unit_CHT_C",4,1) == 1 then DispUnits.CHT = "°C" else DispUnits.CHT = "°F" end
    if Table_ValGet(EngineDamage_Drefs_Once,"Unit_EGT_C",4,1) == 1 then DispUnits.EGT = "°C" else DispUnits.EGT = "°F" end
    if Table_ValGet(EngineDamage_Drefs_Once,"Unit_ITT_C",4,1) == 1 then DispUnits.ITT = "°C" else DispUnits.ITT = "°F" end
    -- Loop through available engines
    for i=1,Table_ValGet(EngineDamage_Drefs_Once,"Eng_Num",4,1) do
        EngineData[i] = {
            {"CHT",-1,-1,-1,-1,0}, -- ID, base limit, lower limit randomized, upper limit randomized, damage tolerance, incurred damage
            {"EGT",-1,-1,-1,-1,0}, -- ID, base limit, lower limit randomized, upper limit randomized, damage tolerance, incurred damage
            {"ITT",-1,-1,-1,-1,0}, -- ID, base limit, lower limit randomized, upper limit randomized, damage tolerance, incurred damage
            {"N1",-1,-1,-1,-1,0}, -- ID, base limit, lower limit randomized, upper limit randomized, damage tolerance, incurred damage
            {"N2",-1,-1,-1,-1,0}, -- ID, base limit, lower limit randomized, upper limit randomized, damage tolerance, incurred damage
            {"TRQ",-1,-1,-1,-1,0}, -- ID, base limit, lower limit randomized, upper limit randomized, damage tolerance, incurred damage
        }
        -- Loop through engine data table and see if the parameters in its subtables can be populated
        for j=1,#EngineData[i] do
            if Table_ValGet(EngineDamage_Drefs_Once,"Limit_"..EngineData[i][j][1],4,1) > 100 or Table_ValGet(EngineDamage_Config_Vars,"DMG_"..EngineData[i][j][1],nil,3) > -1 then
                if Table_ValGet(EngineDamage_Config_Vars,"DMG_"..EngineData[i][j][1],nil,3) > -1 then -- Check if there is a user override for the limit
                    Table_ValSet(EngineData[i],EngineData[i][j][1],nil,2,Table_ValGet(EngineDamage_Config_Vars,"DMG_"..EngineData[i][j][1],nil,3)) -- Assign the user override
                else
                    Table_ValSet(EngineData[i],EngineData[i][j][1],nil,2,Table_ValGet(EngineDamage_Drefs_Once,"Limit_"..EngineData[i][j][1],4,1)) -- Assign the dataref limit
                end
                Table_ValSet(EngineData[i],EngineData[i][j][1],nil,3,EngineDamage_Randomize("RandomizeLimit",Table_ValGet(EngineData[i],EngineData[i][j][1],nil,2))) -- Randomize lower limit a bit
                Table_ValSet(EngineData[i],EngineData[i][j][1],nil,4,EngineDamage_Randomize("RandomizeLimit",Table_ValGet(EngineData[i],EngineData[i][j][1],nil,2) * Table_ValGet(EngineDamage_Config_Vars,"DMG_"..EngineData[i][j][1],nil,4))) -- Randomize upper limit a bit
                Table_ValSet(EngineData[i],EngineData[i][j][1],nil,5,math.floor(EngineDamage_Randomize("RandomizeLimit",Table_ValGet(EngineDamage_Config_Vars,"DMG_"..EngineData[i][j][1],nil,5))))
                LogOutput("Aircraft engine "..i.." CHT damage range: "..Table_ValGet(EngineData[i],EngineData[i][j][1],nil,3).." "..DispUnits.CHT.." to "..Table_ValGet(EngineData[i],EngineData[i][j][1],nil,4).." "..DispUnits.CHT..", Tolerance: "..Table_ValGet(EngineData[i],EngineData[i][j][1],nil,5))
            end
        end
    end
end
...
function EngineDamage_Menu_Callbacks(itemref)
    for i=2,#EngineDamage_Menu_Items do
        if itemref == EngineDamage_Menu_Items[i] then
            ...
            if DebugIsEnabled() == 1 then Debug_Restart() end
        end
    end
end
...
function EngineDamage_Menu_Watchdog(intable,index)
    ...
    if index == 4 then
        ...
        if Table_ValGet(EngineData[1],"CHT",nil,2) > -1 then Menu_ChangeItemSuffix(EngineDamage_Menu_ID,index,"("..Table_ValGet(EngineData[1],"CHT",nil,2).." "..DispUnits.CHT..")",intable) end
    end
    if index == 5 then
        if Table_ValGet(EngineDamage_Config_Vars,"DMG_EGT",nil,2) == 1 then Menu_CheckItem(EngineDamage_Menu_ID,index,"Activate")
        else Menu_CheckItem(EngineDamage_Menu_ID,index,"Deactivate") end
        if Table_ValGet(EngineData[1],"EGT",nil,2) > -1 then Menu_ChangeItemSuffix(EngineDamage_Menu_ID,index,"("..Table_ValGet(EngineData[1],"EGT",nil,2).." "..DispUnits.EGT..")",intable) end
    end
    if index == 6 then
        if Table_ValGet(EngineDamage_Config_Vars,"DMG_ITT",nil,2) == 1 then Menu_CheckItem(EngineDamage_Menu_ID,index,"Activate")
        else Menu_CheckItem(EngineDamage_Menu_ID,index,"Deactivate") end
        if Table_ValGet(EngineData[1],"ITT",nil,2) > -1 then Menu_ChangeItemSuffix(EngineDamage_Menu_ID,index,"("..Table_ValGet(EngineData[1],"ITT",nil,2).." "..DispUnits.ITT..")",intable) end
    end
    if index == 7 then
        if Table_ValGet(EngineDamage_Config_Vars,"DMG_N1",nil,2) == 1 then Menu_CheckItem(EngineDamage_Menu_ID,index,"Activate")
        else Menu_CheckItem(EngineDamage_Menu_ID,index,"Deactivate") end
        if Table_ValGet(EngineData[1],"N1",nil,2) > -1 then Menu_ChangeItemSuffix(EngineDamage_Menu_ID,index,"("..Table_ValGet(EngineData[1],"N1",nil,2).." "..DispUnits.N1..")",intable) end
    end
    if index == 8 then
        if Table_ValGet(EngineDamage_Config_Vars,"DMG_N2",nil,2) == 1 then Menu_CheckItem(EngineDamage_Menu_ID,index,"Activate")
        else Menu_CheckItem(EngineDamage_Menu_ID,index,"Deactivate") end
        if Table_ValGet(EngineData[1],"N2",nil,2) > -1 then Menu_ChangeItemSuffix(EngineDamage_Menu_ID,index,"("..Table_ValGet(EngineData[1],"N2",nil,2).." "..DispUnits.N2..")",intable) end
    end
    if index == 9 then
        if Table_ValGet(EngineDamage_Config_Vars,"DMG_TRQ",nil,2) == 1 then Menu_CheckItem(EngineDamage_Menu_ID,index,"Activate")
        else Menu_CheckItem(EngineDamage_Menu_ID,index,"Deactivate") end
        if Table_ValGet(EngineData[1],"TRQ",nil,2) > -1 then Menu_ChangeItemSuffix(EngineDamage_Menu_ID,index,"("..Table_ValGet(EngineData[1],"TRQ",nil,2).." "..DispUnits.TRQ..")",intable) end
    end
end
