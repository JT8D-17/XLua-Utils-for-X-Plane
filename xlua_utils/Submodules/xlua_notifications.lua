...
function NotificationTimer()
    if XPLM.XPLMGetWindowIsVisible(XluaUtils_DebugWinID) == 1 then
        UpdateNotification("Debug window open, time: "..os.date("%x, %H:%M:%S"),"Nominal",-10)
    elseif XPLM.XPLMGetWindowIsVisible(XluaUtils_DebugWinID) == 0 and CheckNotification(-10) then
        RemoveNotification(-10)
    end
    if #Notification_Stack > 0 then XPLM.XPLMSetWindowIsVisible(XluaUtils_NotifyWinID,1) else XPLM.XPLMSetWindowIsVisible(XluaUtils_NotifyWinID,0) end
end
--[[ Draw callback for the notification window ]]
function NotifyWin_Draw(inWindowID,inRefcon)
    ...
    Window_Coords_Set(XluaUtils_NotifyWinID,Window_Coords)
...
function NotificationWindow_Init()
    ...
    XLuaUtils_NotifyWin.drawWindowFunc = NotifyWin_Draw
    ...
    XluaUtils_NotifyWinID = XPLM.XPLMCreateWindowEx(ffi.cast("XPLMCreateWindow_t *",XLuaUtils_NotifyWin))
    if XluaUtils_NotifyWinID ~= nil then -- Do after window creation
        XPLM.XPLMSetWindowTitle(XluaUtils_NotifyWinID,ffi.new("char[256]",Window_Title))
        PrintToConsole("Notification window created! (ID: "..tostring(XluaUtils_NotifyWinID)..")")
